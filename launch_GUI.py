...
pressure_tab.add_plot(title=inner_vessel_plot_title, x_axis=('Time since present', 's'), y_axis=('Pressure', 'Torr'), offset=[inner_vessel_pressure_offset], buffer_size=10, data_filepaths=[inner_vessel_pressure_log_filepath], datatypes=['inner_vessel_pressure'])
...
pressure_tab.add_plot(title=both_vessels_plot_title, x_axis=('Time since present', 's'), y_axis=('Pressure', 'Torr'), offset=[outer_vessel_pressure_g1_offset, outer_vessel_pressure_g2_offset, inner_vessel_pressure_offset], buffer_size=10, data_filepaths=[outer_vessel_pressure_log_filepath, outer_vessel_pressure_log_filepath, inner_vessel_pressure_log_filepath], datatypes=['outer_vessel_gauge_1_pressure', 'outer_vessel_gauge_2_pressure', 'inner_vessel_pressure'])
pressure_tab.start_timer(title=both_vessels_plot_title, interval_ms=interval_time_ms)

gauge_pressure_plot_title = 'Plot Gauge Pressure'
...
pressure_tab.add_plot(title=gas_flowrate_plot_title, x_axis=('Time since present', 's'), y_axis=('Flowrate', 'L/min'), offset=[gas_flow_offset], buffer_size=10, data_filepaths=[gas_flow_log_filepath], datatypes=['flowrate'])
...
pressure_ctrl_titles = [inner_vessel_plot_title, outer_vessel_plot_title, both_vessels_plot_title, gauge_pressure_plot_title, gas_flowrate_plot_title]
...
for i in range(0, num_vmms):
    temp_tab.add_plot(title=f'Plot VMM {i} Temperature', x_axis=('Time since present', 's'), y_axis=('Temperature', 'deg C'), offset=[vmm_temperatures_offset], buffer_size=10, data_filepaths=[vmm_temperatures_log_filepath], datatypes=['temperature'], vmm_nums=[i])
